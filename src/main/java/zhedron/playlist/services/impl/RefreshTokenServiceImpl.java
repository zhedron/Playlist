...
public class RefreshTokenServiceImpl implements RefreshTokenService {
    ...
    private final long TOKEN_EXPIRATION_TIME = 30 * 24 * 60 * 60 * 1000L;
    ...
    public RefreshToken generateRefreshToken(String email) {
        ...
        if (!refreshTokenRepository.existsByUser(optionalUser.get())) {
            ...
            refreshToken.setExpiredAt(new Date(System.currentTimeMillis() + TOKEN_EXPIRATION_TIME));

            return refreshTokenRepository.save(refreshToken);
        } else {
            RefreshToken foundRefreshToken = refreshTokenRepository.findByUser(optionalUser.get());

            foundRefreshToken.setExpiredAt(new Date(System.currentTimeMillis() + TOKEN_EXPIRATION_TIME));
